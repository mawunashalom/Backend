/*
------------------------------------------------------------------------------ 
This code was generated by Amplication. 
 
Changes to this file will be lost if the code is regenerated. 

There are other ways to to customize your code, see this doc to learn more
https://docs.amplication.com/how-to/custom-code

------------------------------------------------------------------------------
  */
import { InputType, Field } from "@nestjs/graphql";
import { OrdonnanceWhereUniqueInput } from "../../ordonnance/base/OrdonnanceWhereUniqueInput";
import { ApiProperty } from "@nestjs/swagger";

@InputType()
class OrdonnanceUpdateManyWithoutDossierMedicalsInput {
  @Field(() => [OrdonnanceWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [OrdonnanceWhereUniqueInput],
  })
  connect?: Array<OrdonnanceWhereUniqueInput>;

  @Field(() => [OrdonnanceWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [OrdonnanceWhereUniqueInput],
  })
  disconnect?: Array<OrdonnanceWhereUniqueInput>;

  @Field(() => [OrdonnanceWhereUniqueInput], {
    nullable: true,
  })
  @ApiProperty({
    required: false,
    type: () => [OrdonnanceWhereUniqueInput],
  })
  set?: Array<OrdonnanceWhereUniqueInput>;
}

export { OrdonnanceUpdateManyWithoutDossierMedicalsInput as OrdonnanceUpdateManyWithoutDossierMedicalsInput };
